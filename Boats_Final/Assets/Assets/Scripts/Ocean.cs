...
namespace PhillipsOcean
{
	...
	{	
		...
		void Start() 
		{
			...
			for(int x = 0; x < m_numGridsX; x++)
			{
				for(int z = 0; z < m_numGridsZ; z++)
				{
                    ...
                    GameObject ChildGO = new GameObject("Meshcollider from " + idx.ToString());
                    ChildGO.transform.parent = m_oceanGrid[idx].transform;
                    ChildGO.AddComponent<MeshCollider>();
					ChildGO.tag = "WaterCollider";
                    ChildGO.transform.position= new Vector3(ChildGO.transform.position.x, ChildGO.transform.position.y-2, ChildGO.transform.position.z);
			...
			CreateFresnelLookUp();
            float offsetX =m_numGridsX / 2f * m_length;
            float offsetZ = m_numGridsZ / 2f * m_length;
		...
		void Update () 
		{
            ...
            m_oceanGrid[20].GetComponentInChildren<MeshCollider>().sharedMesh = null;
            m_oceanGrid[20].GetComponentInChildren<MeshCollider>().convex = false;
            m_oceanGrid[20].GetComponentInChildren<MeshCollider>().sharedMesh = m_mesh;
            m_oceanGrid[20].GetComponentInChildren<MeshCollider>().sharedMesh.RecalculateBounds();
